...
2. 開発用インスタンスでFPGAイメージの元データ(DCP)をビルドし、S3にアップロードしてFPGAイメージを作成
...
| 用途 | インスタンス | 料金 | vCPU数 | メモリ | cl_hello_world のDCP作成時間 |
| --- | --- | --- | --- | --- | --- |
...
時間に関しては、測定回数が少ないので参考値です。(z1dインスタンスは、EC2インスタンスで最高速の最大4.0GHzのクロック数のインスタンスなので高速になります)
